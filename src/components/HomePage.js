...
//import 3Picker from "emoji-picker-react";
...
const HomePage = (props) => {
  ...
  //const [chosenEmoji, setChosenEmoji] = setState(null);
  let unsubscribe;
    ...
    <Layout>
      <section className="container">
        <div className="listOfUsers">
          {user.users.length > 0
            ? user.users.map((user) => {
                return <User onClick={chatInit} key={user.uid} user={user} />;
              })
            : null}
        </div>
              ...
              ? user.conversations.map((con) => (
                  ...
                  >
                    <p
                      style={{
                        backgroundColor:
                          con.user_uid_1 === auth.uid ? "#D1D5DB" : "#fca5a5",
                      }}
